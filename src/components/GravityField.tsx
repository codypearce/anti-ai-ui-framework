...
export const GravityField: React.FC<GravityFieldProps> = ({
  ...
  const [items, setItems] = useState<DriftingItem[]>(() =>
  ...
  useEffect(() => {
    ...
    wellsRef.current = generateWells(
      wellCount,
  ...
  const updatePositions = useCallback(() => {
    ...
    // Sync well positions for rendering
    if (showWells) {
      setWellPositions([...wellsRef.current]);
    }

    animationRef.current = requestAnimationFrame(updatePositions);
  }, [noise, updateInterval, showWells]);
  ...
  }, [reseedOnHover, wellCount, wellStrength, logger]);
